...
2. desc 为一段连贯的自然语言，长度 500~600 字，不得简略，
   ...
   **人物动作与身体互动必须忠实于正文并写全**：正文明确发生的动作（做爱、口交、
   插入、抚摸、亲吻身体部位等）必须在 desc 中具体写出，禁止省略、弱化或降级为
   普通画面（拥抱、对视、并肩、普通接吻）；正文写多细就写多细，宁夸张勿收敛。
3. tags 为英文 Danbooru 标签串，长度 300~400 字符，以英文逗号分隔，
   ...
   **tag 数量：越多越好，至少 50 个，理想 50~60 个**（主体 1~2、外观 8~12、
   服装 8~12、动作与身体部位 15~25、场景 4~6、画风 4~6），尽量让标签串长度贴近
   300~400 字符；禁止用十几个以内的短串糊弄，角色服饰、肢体动作、表情、
   场景细节、光线氛围尽量拆细，宁多勿少，能写多细写多细。
   **必须忠实于正文，禁止降级：** 正文明确写出的动作、身体部位、接触与互动关系，
   desc 与 tags 必须原样、完整地反映 —— 正文写做爱/插入/口交，就写对应动作与部位，
   绝不能为了「含蓄、美感、画面感」把它省略、弱化或改成普通画面（拥抱、对视、并肩、普通接吻）。
   漏写或降级正文明确发生的内容 = 严重失误。宁可夸张，不可收敛；正文写多细，提示词写多细。
 ...
 * meta.nsfwMix 为分流通道的内容组合（'daily_nsfw' | 'nsfw_only'）：
 *   daily_nsfw = 出两张时：一张日常 + 一张 NSFW（nsfw_max=1 时退化为只出 NSFW）；
 *   nsfw_only  = 全部画面都是 NSFW。
 * meta.nsfwDailyPlace 为「日常那一张」的画面位置（'auto'|'front'|'middle'|'end'），
 * 由分析模型据此从正文对应区段挑一个非成人时刻；NSFW 那一张永远由语义判断。
 *
...
export function buildAnalysisParts(reply, context, max, meta = {}) {
    ...
    if (meta.nsfw) {
        ...
        const placeHint = dailyPlace === 'auto'
            ? '位置不限，由你自行挑一个非成人时刻'
            : ({ front: '取自正文前段', middle: '取自正文中段', end: '取自正文后段' })[dailyPlace];
        if (wantDaily) {
            parts.push(`【本次画面要求】\n本次正文已判定为成人向（NSFW），将走专门的成人向出图通道。`
                + `本次共输出 ${n} 张：其中 **1 张为日常画面**（非成人内容，${placeHint}，`
                + `不得包含成人行为），**另外 ${n - 1} 张为成人画面**（真正发生亲密 / 成人行为`
                + `的那一时刻，位置由你语义判断，前段、中段或后段都可能）。`
                + `成人画面不得为了「画面感强」去选日常铺垫或普通对话；`
                + `日常画面不得为了省事混入成人内容。\n`);
        } else if (mix === 'nsfw_only') {
            parts.push(`【本次画面要求】\n本次正文已判定为成人向（NSFW），将走专门的成人向出图通道。`
                + `本次共输出 ${n} 张，**全部必须是成人画面** —— 各自对应正文中不同时刻的`
                + `真正亲密 / 成人行为（位置由你语义判断，前段、中段或后段都可能），`
...
/**
 * splitProseChunks 把正文按段切分并聚成 n 份（直出多张时，每份送一次上游）。
 *
 * 直出模式要出多张时，不能把整段正文重复送 N 遍（会得到 N 张几乎一样的图）。
 * 用户做法是「开多个窗口」：第 1 张发前段、第 2 张发中段…… 每张独立请求，内容天然不同。
 *
 * 切分口径：先按空行分段；若整段只有一个「大段」（AI 正文常用单换行分隔段落），
 * 退化为按单换行再切；若连换行都几乎没有，则按字符数均分 —— 保证总能切出多份，
 * 而不是把整条正文当成一段（那正是「设置了 N 张却永远只出一张」的根源）。
 *
 * @param {string} text 正文原文（stripMarkers 之后）
 * @param {number} n 期望份数（>=1）
 * @returns {string[]} 最多 n 份正文切片；正文为空返回 []
 */
export function splitProseChunks(text, n) {
    ...
    // 2) 空行分段失败 → 单换行分段（AI 常用连续单换行分隔段落）。
    if (paras.length === 1 && /[\r\n]/.test(src)) {
        paras = src.split(/\r?\n+/).map(p => p.trim()).filter(Boolean);
    }
    // 3) 仍只有一段（没有换行的长文）→ 按字符数均分，从句子边界附近切。
    ...
    // 段落多于份数：按段落数均分，每份拿到 ceil 或 floor 段，尽量均衡。
    const chunks = [];
...
/**
 * applyProseMarkers 把多个直出标记分别插到正文各段落下。
 *
 * 与 applyProseMarker 的单标记版对应：items 每项 { prose, at }，
 * at 为该标记应插到哪个下标之前（由 splitProseChunks 配段后算出）。
 * 从后往前插，避免先插的标记改变后面下标。at 越界或为负时挂末尾。
 *
 * @param {string} text 正文原文
