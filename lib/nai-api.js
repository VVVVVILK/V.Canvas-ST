...
export async function generateIllustration({
    ...
    if (!base) throw new Error('未配置 NAI 服务地址：请先启动一个 NovelAI 协议服务（如 V.Adapter），并把地址填进扩展设置');
    ...
    try {
        ...
        throw new Error(`连不上 NAI 服务（${base}）：${err?.message ?? err}。请确认适配服务已启动、地址正确，且允许跨域（CORS）`);
    }
