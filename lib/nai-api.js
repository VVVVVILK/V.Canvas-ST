 ...
 * @param {string} [p.salt] V站 共享盐覆盖值（留空用内置默认）
 * @param {'off'|'auto'|'on'} [p.signMode] 签名开关：off=从不发（默认）/ auto=仅 vcs_ 专属密钥发 / on=任何密钥都发
 * @param {boolean} [p.allowBridge=true] 连不上填写的地址时，是否回退到同页面的 V.Adapter 桥。
 *   分流通道必须传 false：桥的另一端是主通道的上游，那正是这次要绕开的对象 ——
 *   把分流画面的提示词交给它，等于分流白做，还会把内容送到不该送的地方。
 * @returns {Promise<{base64?:string, url?:string, extension?:string, via?:string, expand?:string, prompt?:string}>}
 *   base64/extension 为本地字节结果；url 存在时表示远程链接降级结果（无本地字节，直接引用该链接）
 */
export async function generateIllustration({
    ...
    if (usePageBridge) {
        ...
        // 200 + url 且无字节：V.Adapter 的 CORS 降级结果 —— 上游只给出远程图片链接，
        // 浏览器侧因跨域无法下载字节。原样把链接交回调用方，由调用方直接引用远程图。
        if (r.status === 200 && !r.bytes && r.url) {
            clearTimeout(timer);
            return { url: String(r.url), via: String(r.via ?? ''), expand: '', prompt: '' };
        }
        // 200 但既无字节也无链接：结果为空，按失败处理。
        // （不能放行，否则空结果会被当作 200 响应继续走图片解析，产出损坏的图片文件。）
        if (r.status === 200 && !r.bytes) {
            clearTimeout(timer);
            throw new Error('V.Adapter 页面内调用返回了空结果');
        }
        ...
        try {
            /* 站点当前盐：只有 V站 专属密钥才需要，且只在没有手动指定盐时才去问。
             * 站点轮换 exclusiveSalt 后，插件下次出图自动跟随，不必更新插件版本。
             * 官方 NAI / 第三方中转（密钥不是 vcs_ 开头）不会走到这里，零额外请求。 */
            let siteSalt = String(salt ?? '').trim();
            if (!siteSalt && signMode !== 'off' && isExclusiveKey(apiKey)) {
                // 取不到就用 vsig 里的兜底值（见 sigHeaders）
            ...
            resp = await fetch(`${base}/ai/generate-image${qs}`, {
                ...
                headers: {
                    ...baseHeaders,
                    // 仅 V站 专属密钥会带签名头；其它上游展开为空对象，行为完全不变。
            ...
            /* 站点换盐的自愈：盐在会话内缓存，若站点刚轮换过盐，手里的旧盐会被 403。
             * 此时清掉缓存重新问一次，拿到新盐后重试一次即可 —— 用户不必刷新页面，
             * 也不必更新插件。（仅在没有手动指定盐时才自愈，手动指定以用户填写为准。） */
            ...
            if (pageBridge) {
                try {
                    ...
                    if (r2 && typeof r2 === 'object' && r2.status === 200 && r2.bytes) {
                        ...
                        bridgeErr = (r2 && r2.error) || `页面内调用返回 ${r2?.status ?? '空结果'}`;
                    }
                } catch (e2) {
            ...
            if (!resp) {
                // 桥存在且给出了明确失败原因时，优先透传真实原因
                // （最常见：V.Adapter 自己的上游还没有配置）。
                if (bridgeErr) {
                    throw new Error(`V.Adapter（页面内直连）出图失败：${bridgeErr}`);
                }
 ...
 * 专属密钥同样带上签名：V站 当前不校验该端点，但将来收紧时这里无需再改。
